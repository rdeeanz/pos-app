...
import { 
  ...
  X, 
  Command,
  Monitor // Ikon tambahan untuk POS
} from "lucide-react";
...
export default function AdminNavbar({ user }) {
  ...
  const navItems = [
    ...
    { href: "/pos", label: "Open POS", icon: Monitor }, // Menu POS baru
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/admin" className="flex items-center gap-2.5 group">
            ...
            <div>
              <h1 className="text-sm font-bold text-zinc-900 tracking-tight leading-none">POS Admin</h1>
              <p className="text-[10px] font-medium text-zinc-500 uppercase tracking-wider">Management</p>
            </div>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-1">
            {navItems.map((item) => {
              // Logika perbaikan agar Dashboard tidak terus-menerus menyala
              ...
              return (
                ...
                >
                  <Icon size={18} strokeWidth={isActive ? 2.5 : 2} className={isActive ? "text-zinc-900" : "text-zinc-400"} />
          ...
          <div className="hidden md:flex items-center gap-4 pl-4 border-l border-zinc-200">
            <div className="flex items-center gap-3">
              <div className="text-right hidden lg:block">
                <p className="text-sm font-semibold text-zinc-900 leading-none">{user?.email?.split('@')[0]}</p>
                <p className="text-xs text-zinc-500 mt-1 capitalize">{user?.role || 'Admin'}</p>
              </div>
